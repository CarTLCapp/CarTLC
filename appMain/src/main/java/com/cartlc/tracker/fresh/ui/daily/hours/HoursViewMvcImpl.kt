...
class HoursViewMvcImpl(
    private val inflater: LayoutInflater,
    container: ViewGroup?
) : ObservableViewMvcImpl<HoursViewMvc.Listener>(),
    HoursViewMvc,
    ...
    override val rootView: View =
        inflater.inflate(R.layout.content_hours, container, false) as ViewGroup
    ...
    private val dayDisplayView = findViewById<Button>(R.id.dateDisplay)
    private val startTimeView = findViewById<Button>(R.id.timeStart)
    private val endTimeView = findViewById<Button>(R.id.timeStart)
    ...
    private val itemList = findViewById<RecyclerView>(R.id.itemList)
    ...
    init {
        ...
        dayDisplayView.setOnClickListener { listeners.forEach { it.onDateButtonClicked() } }
        itemList.layoutManager = LinearLayoutManager(context)
        projectRecent.setOnCheckedChangeListener { _, isChecked ->
            onProjectSelected(
                ProjectSelect.PROJECT_RECENT,
                isChecked
            )
        }
        projectAll.setOnCheckedChangeListener { _, isChecked ->
            onProjectSelected(
                ProjectSelect.PROJECT_ALL,
                isChecked
            )
        }
        projectOther.setOnCheckedChangeListener { _, isChecked ->
            onProjectSelected(
    ...
    override var dayVisible: Boolean
        get() = dayDisplayView.visibility == View.VISIBLE
        set(value) {
            dayDisplayView.visibility = if (value) View.VISIBLE else View.GONE
        }

    override var dayTextValue: String?
        get() = dayDisplayView.text.toString()
        set(value) {
            dayDisplayView.text = value
        }

    override var startTimeVisible: Boolean
        get() = startTimeView.visibility == View.VISIBLE
        set(value) {
            startTimeView.visibility = if (value) View.VISIBLE else View.GONE
        }

    override var startTimeValue: String?
        get() = startTimeView.text.toString()
        set(value) {
            startTimeView.text = value
        }

    override var endTimeVisible: Boolean
        get() = endTimeView.visibility == View.VISIBLE
        set(value) {
            endTimeView.visibility = if (value) View.VISIBLE else View.GONE
        }

    override var endTimeValue: String?
        get() = endTimeView.text.toString()
        set(value) {
            endTimeView.text = value
        }
    ...
    override var itemListVisible: Boolean
        get() = itemList.visibility == View.VISIBLE
        set(value) {
            itemList.visibility = if (value) View.VISIBLE else View.GONE
        }
    ...
    override fun prepareItemList(items: List<String>) {
        itemList.adapter = ItemAdapter(inflater, items, this)
    }
    ...
    // endregion HoursViewMvc

    // region ItemAdapter.Listener

    override fun onItemSelected(position: Int, item: String) {
        listeners.forEach { it.onItemSelected(position, item) }
    }

    override fun isSelected(position: Int, item: String): Boolean {
        return listeners.firstOrNull()?.isItemSelected(position, item) ?: false
    }

    // endregion ItemAdapter.Listener
