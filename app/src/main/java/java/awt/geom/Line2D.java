    ...
    class Iterator implements PathIterator {
        ...
        public int currentSegment(double[] coords) {
            if (isDone()) {
                ...
                throw new NoSuchElementException("Line2D.currentSegment: no such element"); //$NON-NLS-1$
            }
            int type;
            if (index == 0) {
                type = SEG_MOVETO;
        ...
        public int currentSegment(float[] coords) {
            if (isDone()) {
                // awt.4B=Iterator out of bounds
                throw new NoSuchElementException("Line2D.currentSegment no such element [2]"); //$NON-NLS-1$
            }
            int type;
            if (index == 0) {
                type = SEG_MOVETO;
