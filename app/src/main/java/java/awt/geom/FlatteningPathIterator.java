...
public class FlatteningPathIterator implements PathIterator {
    ...
    public FlatteningPathIterator(PathIterator path, double flatness, int limit) {
        if (flatness < 0.0) {
            ...
            throw new IllegalArgumentException("FlatteningPathIterator: flatness less than zero: " + flatness); //$NON-NLS-1$
        }
        if (limit < 0) {
            // awt.207=Limit is less then zero
            throw new IllegalArgumentException("FlatteningPathIterator: limit less than zero: " + limit); //$NON-NLS-1$
        }
        if (path == null) {
            // awt.208=Path is null
            throw new NullPointerException("FlatteningPathIterator: path NULL"); //$NON-NLS-1$
        }
        this.p = path;
    ...
    public int currentSegment(float[] coords) {
        if (isDone()) {
            ...
            throw new NoSuchElementException("currentSegment: no such element"); //$NON-NLS-1$
        }
        evaluate();
        int type = bufType;
        if (type != SEG_CLOSE) {
    ...
    public int currentSegment(double[] coords) {
        if (isDone()) {
            // awt.4B=Iterator out of bounds
            throw new NoSuchElementException("currentSegment: nbo such element [2]"); //$NON-NLS-1$
        }
        evaluate();
        int type = bufType;
        if (type != SEG_CLOSE) {
