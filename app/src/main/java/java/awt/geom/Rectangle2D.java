...
public abstract class Rectangle2D extends RectangularShape {
    ...
    class Iterator implements PathIterator {
        ...
        public int currentSegment(double[] coords) {
            if (isDone()) {
                throw new NoSuchElementException("No such element in currentSegment[2]"); //$NON-NLS-1$
            }
            if (index == 5) {
                return SEG_CLOSE;
            }
        ...
        public int currentSegment(float[] coords) {
            if (isDone()) {
                throw new NoSuchElementException("No such element in currentSegment[3]"); //$NON-NLS-1$
            }
            if (index == 5) {
                return SEG_CLOSE;
            }
