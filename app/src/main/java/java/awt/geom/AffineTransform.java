...
public class AffineTransform implements Cloneable, Serializable {
    ...
    public AffineTransform createInverse() throws NoninvertibleTransformException {
        ...
        if (Math.abs(det) < ZERO) {
            ...
            throw new NoninvertibleTransformException("Non Invertible Transform exception"); //$NON-NLS-1$
        }
    ...
    public Point2D inverseTransform(Point2D src, Point2D dst)
        ...
        if (Math.abs(det) < ZERO) {
            // awt.204=Determinant is zero
            throw new NoninvertibleTransformException("inverse transform value less than zero"); //$NON-NLS-1$
        }
        if (dst == null) {
            if (src instanceof Point2D.Double) {
                dst = new Point2D.Double();
    ...
    public void inverseTransform(double[] src, int srcOff, double[] dst, int dstOff, int length)
        ...
        if (Math.abs(det) < ZERO) {
            // awt.204=Determinant is zero
            throw new NoninvertibleTransformException("inverse transform value less than zero: " + det); //$NON-NLS-1$
        }
        while (--length >= 0) {
     ...
     * original shape.
     */
    public Shape createTransformedShape(Shape src) throws GeneralPath.IllegalPathStateException {
        if (src == null) {
            return null;
        }
