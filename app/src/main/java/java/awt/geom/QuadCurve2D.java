    ...
    class Iterator implements PathIterator {
        ...
        public int currentSegment(double[] coords) {
            if (isDone()) {
                ...
                throw new NoSuchElementException("QuadCurve2.currentSegment: no such element"); //$NON-NLS-1$
            }
            int type;
            int count;
            if (index == 0) {
        ...
        public int currentSegment(float[] coords) {
            if (isDone()) {
                // awt.4B=Iterator out of bounds
                throw new NoSuchElementException("QuadCurve2D.currentSegment: no such element [2]"); //$NON-NLS-1$
            }
            int type;
            int count;
            if (index == 0) {
