...
public abstract class CubicCurve2D implements Shape, Cloneable {
    ...
    class Iterator implements PathIterator {
        ...
        public int currentSegment(double[] coords) {
            if (isDone()) {
                throw new NoSuchElementException("CubicCurrent2D.currentSegment: no such element"); //$NON-NLS-1$
            }
            int type;
            int count;
            if (index == 0) {
        ...
        public int currentSegment(float[] coords) {
            if (isDone()) {
                throw new NoSuchElementException("CubicCurve2D.currentSegment: no such element [2]"); //$NON-NLS-1$
            }
            int type;
            int count;
            if (index == 0) {
