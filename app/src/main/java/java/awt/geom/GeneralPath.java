...
public final class GeneralPath implements Shape, Cloneable {
    ...
    class Iterator implements PathIterator {
        ...
        public int currentSegment(double[] coords) {
            if (isDone()) {
                ...
                throw new NoSuchElementException("currentSegment: no such element in general path"); //$NON-NLS-1$
            }
        ...
        public int currentSegment(float[] coords) {
            if (isDone()) {
                // awt.4B=Iterator out of bounds
                throw new NoSuchElementException("currentSegment: no such element while in geeneral path [2]"); //$NON-NLS-1$
            }
            int type = p.types[typeIndex];
            int count = GeneralPath.pointShift[type];
     ...
     *
     * @param rule the winding rule, either {@link PathIterator#WIND_EVEN_ODD} or
     ...
     */
    public GeneralPath(Shape shape) throws IllegalPathStateException {
        this(WIND_NON_ZERO, BUFFER_SIZE);
    ...
    public void setWindingRule(int rule) {
        if (rule != WIND_EVEN_ODD && rule != WIND_NON_ZERO) {
            ...
            throw new java.lang.IllegalArgumentException("setWindingRule: invalid value: " + rule); //$NON-NLS-1$
        }
        this.rule = rule;
    }
    ...
    public class IllegalPathStateException extends Exception {
     ...
     * @throws IllegalPathStateException checkMove is true and the path is currently empty.
     */
    void checkBuf(int pointCount, boolean checkMove) throws IllegalPathStateException {
        if (checkMove && typeSize == 0) {
            // awt.20A=First segment should be SEG_MOVETO type
            throw new IllegalPathStateException("checkBuf: illegal path"); //$NON-NLS-1$
        }
     ...
     */
    public void moveTo(float x, float y) throws IllegalPathStateException {
     ...
     * @param y the y coordinate of the next point to append.
     */
    public void lineTo(float x, float y) throws IllegalPathStateException {
        checkBuf(2, true);
     ...
     */
    public void quadTo(float x1, float y1, float x2, float y2) throws IllegalPathStateException {
        checkBuf(4, true);
     ...
     *
     * @param x1 the x coordinate of the new cubic segment's first control
     *           point.
     * @param y1 the y coordinate of the new cubic segment's first control
     *           point.
     * @param x2 the x coordinate of the new cubic segment's second control
     *           point.
     * @param y2 the y coordinate of the new cubic segment's second control
     *           point.
     ...
     */
    public void curveTo(float x1, float y1, float x2, float y2, float x3, float y3) throws IllegalPathStateException {
        checkBuf(6, true);
     ...
     * the curve.
     */
    public void closePath() throws IllegalPathStateException {
     ...
     * @param connect true to connect this path's current endpoint to the first
     ...
     */
    public void append(Shape shape, boolean connect) throws IllegalPathStateException {
        PathIterator p = shape.getPathIterator(null);
        append(p, connect);
    }
     ...
     *                outline without connecting it.
     */
    public void append(PathIterator path, boolean connect) throws IllegalPathStateException {
        while (!path.isDone()) {
