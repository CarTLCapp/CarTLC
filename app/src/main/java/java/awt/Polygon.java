...
public class Polygon implements Shape, Serializable {
    ...
    class Iterator implements PathIterator {
        ...
        public int currentSegment(double[] coords) {
            if (isDone()) {
                ...
                throw new NoSuchElementException("Polygon.currentSegment: no such element"); //$NON-NLS-1$
            }
            if (index == p.npoints) {
                return SEG_CLOSE;
            }
        ...
        public int currentSegment(float[] coords) {
            if (isDone()) {
                // awt.110=Iterator out of bounds
                throw new NoSuchElementException("Polygon.currentSegment: no such element [2]"); //$NON-NLS-1$
            }
            if (index == p.npoints) {
                return SEG_CLOSE;
            }
    ...
    public Polygon(int[] xpoints, int[] ypoints, int npoints) {
        if (npoints > xpoints.length || npoints > ypoints.length) {
            ...
            throw new IndexOutOfBoundsException(String.format("Index out of bounds %d %d %d", npoints, xpoints.length, ypoints.length)); //$NON-NLS-1$
        }
        if (npoints < 0) {
            // awt.112=Negative number of points
            throw new NegativeArraySizeException("negative array size: " + npoints); //$NON-NLS-1$
        }
