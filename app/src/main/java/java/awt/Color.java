...
public class Color implements Paint, Serializable {
    ...
    public Color(ColorSpace cspace, float[] components, float alpha) {
        ...
        for (int i = 0; i < nComps; i++) {
            ...
            if (comp < 0.0f || comp > 1.0f) {
                ...
                // {0}.
                throw new IllegalArgumentException(String.format("Color.illegal argument %d : %f", i, comp)); //$NON-NLS-1$
            }
        ...
        if (alpha < 0.0f || alpha > 1.0f) {
            ...
            throw new IllegalArgumentException(String.format("Color.illegal alpha value %f", alpha)); //$NON-NLS-1$
        }
        falpha = alpha;
    ...
    public Color(int r, int g, int b, int a) {
        if ((r & 0xFF) != r || (g & 0xFF) != g || (b & 0xFF) != b || (a & 0xFF) != a) {
            ...
            throw new IllegalArgumentException(String.format("Color.bad rgba values %x %x %x %x", r, g, b, a)); //$NON-NLS-1$
        }
    ...
    public Color(int r, int g, int b) {
        if ((r & 0xFF) != r || (g & 0xFF) != g || (b & 0xFF) != b) {
            // awt.109=Color parameter outside of expected range.
            throw new IllegalArgumentException(String.format("Color.bad rgb values %x %x %x", r, g, b)); //$NON-NLS-1$
        }
